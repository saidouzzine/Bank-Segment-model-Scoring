...
"""
Bank Segment Upgrade Scoring
----------------------------

Pipeline complet de data science pour :
- nettoyer les données,
- créer des variables dérivées,
- encoder les variables catégorielles,
- entraîner plusieurs modèles supervisés,
- sélectionner le meilleur modèle selon l’AUC,
- sauvegarder les modèles entraînés.

Ce script est structuré selon les bonnes pratiques PEP 257.
"""
...
def load_data(path: str) -> pd.DataFrame:
    """
    Charge un fichier CSV séparé par des points-virgules.
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - imputations conditionnelles,
    - gestion des valeurs manquantes,
    - corrections logiques selon les flags.
...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    """
    Crée les variables dérivées :
    - catégories d'âge et d'ancienneté,
    - ratios financiers,
    ...
    df["age_category"] = pd.cut(
        df["age"], bins=[25, 40, 60, 80, 120],
        labels=["25-40", "41-60", "61-80", "81-120"]
    )

    df["anciennete_category"] = pd.cut(
        df["anciennete"], bins=[0, 10, 20, 30, 120],
    ...
    df["ratio_epargne_investie_par_liquide"] = (
        df["encours_epargne_investie"] / (df["encours_epargne_liquide"] + 1)
    )
...
def encode(df: pd.DataFrame) -> pd.DataFrame:
    """
    Encode les variables catégorielles en dummies.
    ...
    df = df.drop(columns=["age", "anciennete"], errors="ignore")
    return df


# -------------------------------------------------------------------
# 5. Modèles et hyperparamètres
# -------------------------------------------------------------------

def get_model_space():
    """
    Définit l’espace des modèles et hyperparamètres.
...
def train_and_select(X_train, y_train, X_val, y_val):
    """
    Entraîne plusieurs modèles et sélectionne celui avec le meilleur AUC.
